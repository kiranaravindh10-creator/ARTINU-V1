import { ANNOUNCEMENT_AUDIENCE_ROLES } from '@artinu/shared';
import type {
...
export async function broadcast(input: {
  ...
  /*
    The audience list grew from three values to eleven, so the mapping lives in
    ANNOUNCEMENT_AUDIENCE_ROLES rather than in a ternary here. `recipientsFor`
    in announcement.routes.ts reads the same table, which is the point: two
    copies of "which roles does `all_internal` mean" is how the two paths start
    disagreeing about who got told.
  */
  const roles = (ANNOUNCEMENT_AUDIENCE_ROLES[input.audience] ?? []) as readonly Role[];
  if (roles.length === 0) return 0;

  // One read with an `in (…)`, the same shape recipientsFor uses.
  const recipients = await db.users.find({
